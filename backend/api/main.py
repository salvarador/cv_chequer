...
class CVTriageRequest(BaseModel):
    text: str
...
class CVTriageResponse(BaseModel):
    ...
    triage: dict
# CV triage endpoint
@app.post("/triage-cv", response_model=CVTriageResponse)
async def triage_cv_api(request: CVTriageRequest):
    triage = triage_cv(request.text)
    return CVTriageResponse(
        success=True,
...
from backend.core.cv_db import init_db, add_checked_cv, is_cv_checked
...
async def analyze_cv(
    ...
    try:
        ...
        # Check if CV is already checked
        if is_cv_checked(file.filename):
            raise HTTPException(status_code=409, detail="CV already checked")

        # Save CV file and metadata
        add_checked_cv(file.filename, content, checked_at, "Pending")
        ...
        # Update result in DB (optional, if you want to store analysis)
        # You can add a function to update the result if needed
